...
const BottomNav = ({ setSidebarOpen }) => {
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-card-bg/95 backdrop-blur-lg border-t border-border-main flex items-center justify-around px-2 py-3 z-50 safe-area-bottom pb-6">
      {navItems.map((item) => {
        ...
        return (
          <button
            ...
            className={`flex flex-col items-center gap-1.5 min-w-[64px] py-1 transition-all active:scale-90 ${
              isActive ? 'text-blue-600' : 'text-text-main'
            }`}
          >
            <item.icon className={`w-6 h-6 ${isActive ? 'fill-blue-600/10 stroke-[2.5px]' : 'stroke-[1.5px]'}`} />
            <span className={`text-[9px] font-black uppercase tracking-tighter ${isActive ? 'text-blue-600' : 'text-text-muted'}`}>
              {item.label}
            </span>
          </button>
        );
